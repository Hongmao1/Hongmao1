  ...
  <div class="order-right">
    <div class="order-content">
            ...
            <tr v-for="(cart, index) in order.orderDetailList" :key="cart.id">
              <td width="60%">
                <div class="typographic">
                  <img :src="cart.imgUrl" style="hight:100px;width:100px">
                  <a href="#" class="block-text">{{cart.skuName}}</a>
                  <span>x{{cart.skuNum}}</span>
                  <a href="#" class="service">售后申请</a>
                </div>
              </td>
              <td :rowspan="order.orderDetailList.length" width="8%" v-if="index == 0"
                class="center">
                {{order.consignee}}
              </td>
              <td :rowspan="order.orderDetailList.length" width="13%" v-if="index == 0"
                class="center">
                <ul class="unstyled">
      ...
      <div class="choose-order">
        <PaginTion :total="myorder.total" :pageNo="page" :pageSize="limit" @getPageNo="getPageNo"
          :continues="5">
        </PaginTion>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async getmyOrder () {
      let res = await this.$API.reqGetmyOrder(this.page, this.limit)
      if (res.code === 200) {
