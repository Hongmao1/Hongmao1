  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="add in addRess" :key="add.id">
        ...
        <p @click="changeDefault(add,addRess)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{defaultAddres.fullAddress}}</span>
        收货人：<span>{{defaultAddres.consignee}}</span>
        <span>{{defaultAddres.phoneNum}}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async toPay () {
      ...
      let data = {
        consignee: this.defaultAddres.consignee,
        consigneeTel: this.defaultAddres.phoneNum,
        deliveryAddress: this.defaultAddres.fullAddress,
        paymentWay: 'ONLINE',
        orderComment: this.msg,
        orderDetailList: this.orderInfo.detailArrayList
      }
      let res = await this.$API.reqSubmitOrder(traderNo, data)
      if (res.code === 200) {
  ...
  computed: {
    ...
    defaultAddres () {
      ...
      return this.addRess.find(state => state.isDefault == 1) || {}
    }
  }
}
</script>
