    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a href="#">综合
                    <span v-show="isOne" class="iconfont icon-direction-down" v-if="isud"></span>
                    <span v-show="isOne" class="iconfont icon-direction-up" v-else></span>
                  </a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a href="#">价格
                    <span v-show="isTwo" class="iconfont icon-direction-down" v-if="isud"></span>
                    <span v-show="isTwo" class="iconfont icon-direction-up" v-else></span>
                  </a>
                </li>
              </ul>
            </div>
          ...
          <!-- 分页器 -->
          <PaginTion :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize" :continues="5"
            :total="total" @getPageNo="getPageNo"></PaginTion>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    removeKeyword () {
      ...
      this.getData()
      this.$bus.$emit('clear')
      // 进行路由跳转,判断是否有query参数有的话带上
      if (this.$route.query) {
        this.$router.push({ name: 'search', query: this.$route.query })
      }
    },
    // 获取商标点击的target并获取数据事件
  ...
  watch: {
    $route () {
      ...
      this.getData()
      // 防止用户选择其他标签的时候id为上次id，请求完了清空id信息
